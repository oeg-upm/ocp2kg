...
class TestRenameEntity00(unittest.TestCase):
    ...
    def test_rename_entity00(self):
        ...
        old_mapping = Graph().parse(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'outdated_shapes.shacl'))
        ontology = Graph()
        review_mappings = Graph()
        updated_mapping, updated_shapes =   ontoripple.propagate(change_data, None, None, None, old_mapping)
...
if __name__ == "__main__":
    unittest.main()
    print("Tests RenameEntity Passed for SHACL Shapes")